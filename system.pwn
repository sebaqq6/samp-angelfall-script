...
#include "modules/item_probably.inc"//Moduł odpowiadajacy za prawdopodobienistwo stworzenia przedmiotu na mapie.
#include "modules/loot.inc"//Moduł odpowiedzialny za rozrzut przedmiotów na mapie
#include "modules/useitem.inc"//Użycie przedmiotu
#include "modules/player.inc"//Operacje na graczu
#include "modules/keys.inc"//Obsługa klawiszy
#include "modules/damage.inc"//Obsługa DMG/HP
...
#include "modules/krypta.inc"//Moduł odpowiadający za funckjonowanie krypty
#include "modules/football.inc"//Piłka nożna
#include "modules/doors.inc"//Obsługa wejść
#include "modules/ognisko.inc"//System ogniska
#include "modules/nick_label.inc"//Moduł odpowiadający6 za nicki nad głowami graczy
#include "modules/exp.inc"//Moduł odpowiadający za doświadczenie w grze
#include "modules/zombie.inc"//Moduł odpowiadający za szwędaczy
#include "modules/commands.inc"//Wszystkie inne komendy, które nie mogą znaleźć się w danym module
#include "modules/fadelabels.inc"//Moduł odpowiadający za zanikające 3d texty
#include "modules/gym.inc"//Moduł obsługujacy siłownie.
#include "modules/combine_mode.inc"//Moduł obsługujacy tryb kombinacji.
#include "modules/vehicle.inc"//Moduł obsługujacy pojazdy.
#include "modules/group.inc"//Moduł obsługujacy grupy.
#include "modules/sms.inc"//Moduł obsługujący mikropłatności.
#include "modules/market.inc"//Moduł obsługujący halę targową.
#include "modules/apartamenty.inc"//Moduł obsługujący apartamenty.
#include "modules/bazy.inc"//Moduł obsługujący bazy grup.
#include "modules/zone.inc"//Moduł obsługujący strefy.
//#include "modules/quest.inc"//Moduł obsługujący questy - INCOMPLETE
//*******************BRUDNOPIS***************************
/*
- Admin Jail 
- awanse na czacie globalnym
- osiągniecia - pierwszy raz zrobiłeś taki 
...
public OnGameModeInit()
{
	...
	CreateDynamic3DTextLabel("***sprawny dystrybutor***\n"COL_WHITE"(("COL_EASY"/tankuj lub użyj kanistra"COL_WHITE"))", 0x765898FF, -1607.9446,-2717.4119,48.5391, 3.0);
	CreateDynamic3DTextLabel("***sprawny dystrybutor***\n"COL_WHITE"(("COL_EASY"/tankuj lub użyj kanistra"COL_WHITE"))", 0x765898FF, -11.9189,-2522.2725,36.6555, 3.0);
	CreateDynamic3DTextLabel(""COL_RED"(("COL_EASY"Naprawdę, poświęć chwilę na "COL_GREEN"/pomoc"COL_LIME" -> "COL_GREEN"Opis rozgrywki"COL_EASY"\n"COL_EASY"Będzie Ci łatwiej zacząć grę."COL_RED"))", 0x765898FF, -2350.1819,-1632.1136,2310.4917, 5.0, _, _, 1);
	CreateDynamic3DTextLabel(""COL_RED"(("COL_ORANGE"Chodź na "COL_CYAN"C"COL_ORANGE" aby generować najmniejszy hałas.\nSpowoduje to, że szwędacze nie będą Cię słyszeć,"COL_RED"))", 0x765898FF, -1995.1685,-1594.2118,87.3582, 10.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -1365.1147,-2000.5859,1.8554, 6.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -1506.7386,-2312.7712,1.8554, 6.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -1520.9185,-2223.4324,2.4344, 6.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -2701.3765,-2197.5000,1.2692, 6.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -1643.8407,-1682.8496,1.6915, 6.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -1241.6897,-2526.8713,1.6894, 6.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -1803.0276,-2766.0796,1.4611, 6.0);
	CreateDynamic3DTextLabel("***molo***\n"COL_WHITE"(("COL_EASY"/wedkuj lub użyj wędki"COL_WHITE"))", 0x765898FF, -381.9236,-1852.0017,1.9192, 6.0);
...
public OnPlayerPickUpPickup(playerid, pickupid)
{
	/*if(pickupid == pick_prezent)
	{
		...
		if(!gracz[playerid][prezent])
		{
			...
			strcat(info_prezent, "\n"COL_GREY"Wszystkiego dobrego z okazji nadchodzących świąt życzy Ekipa {f12f2f}Angel Fall"COL_GREY"!");
...
CMD:autorzy(playerid, params[])
{
	...
	MessageGUIEx(playerid, "Osoby, które zaangażowały się w projekt AngelFall", autorzy);
	return 1;
}
...
stock Process_ServerRestart()
{
	...
	else if(serwer[time_hour] == 3 && serwer[time_minute] == 0)
	{
		...
		SendClientMessageToAllEx(-1, ""COL_RED"-|{fb9800} Za "COL_WHITE"%d{fb9800} sekund nastąpi restart serwera. Powód: "COL_WHITE"Jest godzina: %02d:%02d{fb9800} "COL_RED"|-", serwer[cooldown_restart], serwer[time_hour], serwer[time_minute]);
	}
	return 1;
}
...
{
	new carid, k[2];
	if(sscanf(params,"ddd", carid, k[0], k[1])) return SendClientMessage(playerid, -1, ""COL_GREY"UŻYJ: /car [carid] [k1] [k2] [pozycje x,y,z]");
...
CMD:vipcheck(playerid, params[])
{
	...
	if(isvip(playerid)) SendClientMessage(playerid, -1, "jesteś vipem");
	return 1;
}
